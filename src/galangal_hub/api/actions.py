...
async def get_output_lines(
    ...
    # Try canonical DB-backed task logs for active tasks, but fall through
    # to in-memory output if the TaskIndex has no data (e.g., remote agents
    # where the DB is on a different machine).
    if agent.task:
        try:
